function[Pval,CVM,CVMFD]=Emp_Cop_Test_mid(N,D,NBoot,X)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Empirical Copula test mid version
...
for d1=1:D
[dCdasFD]=FDDi_fast_imp_exp(U2,gridpoints,d1);
[dCdbsFD]=FDDi_fast_imp_exp(U2surv,gridpoints,d1);
ID(d1).dCdFD= dCdasFD/2 + dCdbsFD/2;
corraFD=corraFD+[zeros(1,size(gridpoints,1));repmat(sparse(ID(d1).dCdFD'),NBoot,1)].*squeeze(Cad(:,:,d1))' ;
corrbFD=corrbFD+[zeros(1,size(gridpoints,1));repmat(sparse(ID(d1).dCdFD'),NBoot,1)].*squeeze(Cbd(:,:,d1))' ;
end
